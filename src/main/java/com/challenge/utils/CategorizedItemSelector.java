...
public class CategorizedItemSelector implements Selector<ItemStack> {
  ...
  public CategorizedItemSelector(
      MinecraftServer server,
      Map<ItemCategory, List<Item>> categorizedItems,
      List<ResourceKey<Enchantment>> enchantments) {
    this.server = server;
    this.categorizedItems = categorizedItems;
    this.enchantmentKeys = enchantments;
    this.size =
        categorizedItems.values().stream()
            .map(itemList -> itemList.size())
            .reduce(0, (acc, size) -> Math.max(acc, size));
  }
  ...
  protected ItemStack getItemStackForEnchantedBook(final int modifier) {
    ...
    ItemEnchantments.Mutable itemEnchantments =
        new ItemEnchantments.Mutable(ItemEnchantments.EMPTY);
    for (int i = 0; i < enchantmentNum; i++) {
      int enchantmentModifier = (modifier + i) * (i + 1);

      ResourceKey<Enchantment> enchantmentKey =
          enchantmentKeys.get(enchantmentModifier % enchantmentKeys.size());
      ...
      int enchantmentLevel =
          Math.max(minEnchantmentLevel, (enchantmentModifier % maxEnchantmentLevel));
