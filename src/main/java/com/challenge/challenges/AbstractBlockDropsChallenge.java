    ...
    BlockEvents.AFTER_PLAYER_DESTROY_EVENT.register(
        (world, player, pos, state, blockEntity, tool, isBlockReplaced) -> {
          try {
            ...
            // Make sure to spawn the item at the canter of the original block, because otherwise it
            // will glitch into the blocks around
            Vec3 blockCenterPos = pos.getCenter();
            ItemEntity itemEntity =
                new ItemEntity(
                    world, blockCenterPos.x(), blockCenterPos.y(), blockCenterPos.z(), itemStack);
            itemEntity.setDefaultPickUpDelay();
            world.addFreshEntity(itemEntity);
            return true;
          } catch (Exception e) {
            LOGGER.error(
                "Exception occured while trying to replace block drop for {}: {}",
