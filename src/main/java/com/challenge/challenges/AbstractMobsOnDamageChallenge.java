        ...
        (entity, source, baseDamageTaken, damageTaken, blocked) -> {
          try {
            ...
            if (entity.getType().equals(EntityType.ENDER_DRAGON)
                && level.dimensionType().hasEnderDragonFight()) return;

            // If we got this far, this means that
            // 1. The entity is alive and was not killed by the damage (this is implied by the event
            // definition)
            ...
            LivingEntity replacement =
                (LivingEntity)
                    replacementType.spawn(
                        (ServerLevel) level, entityPos.above(), EntitySpawnReason.TRIGGERED);

            // This is the After Damage event, which means that the damage was already applied to
            // the entity.
            // Therefore, health of the replacement is set to the entity health, and will be
            // automatically set to the max health of the replacement
            ...
            LOGGER.error(
                "Exception occured while trying to replace entity {}: {}", entity.toString(), e);
            return;
          }
        });
  }
